...
class SentenceGather(nn.Module):
    # Word-level gather
    def __init__(self, pool='avg', embed_dim=512):
        super().__init__()
        if pool == 'avg':
            self.pool = lambda x: torch.mean(x, dim=0)
        elif pool == 'max':
            self.pool = lambda x: torch.max(x, dim=0)
        elif pool == 'min':
            self.pool = lambda x: torch.min(x, dim=0)
        elif pool == 'attention':
        ...
        for b in range(x.shape[0]):
            ...
            for i in range(torch.max(roi_mask_per_text).item()):
                ...
                if aggregate_word.size(0) == 0:
                    continue
                roi_words_per_text.append(aggregate_word.mean(0, keepdim=True))
